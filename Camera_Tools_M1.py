...
sources = {
    ...
    "a7c" : ['/Volumes/a7c/DCIM'], # Sony a7c
}

skip_types           = ['LRF','xmp'] # Skip the files with these types.
JPG_support_types    = ['JPG'] # JPG files will be copied to the LIBRARY_PATH.
RAW_support_types    = ['NEF','ARW'] # RAW files will be copied to the LIBRARY_PATH.
Videos_support_types = ['MOV','MP4'] # Videos files will be copied to the LIBRARY_PATH.

LIBRARY_PATH = '/Users/guangmingzeng/Pictures/Photography Temp/Recently' # The files will be copied to this path.
EDITED_PATH  = '/Users/guangmingzeng/Pictures/Adobe Lightroom' # The files in this path will be moved to the LIBRARY_PATH.
LR_RAW_PATH  = "/Users/guangmingzeng/Pictures/Photography Temp/Raw_from_Lr" # Where the RAW files are stored in the Lightroom.
...
def process_jpg(file_location, tag):
    ...
    destination_path = LIBRARY_PATH + "/Day " + DAY + "/" + "JPG_Preview"
...
def process_video(file_location, tag):
    ...
    destination_path = LIBRARY_PATH + "/Day " + DAY + "/" + "Mov"
    if not os.path.exists(destination_path):
        os.makedirs(destination_path)
    destination_file = destination_path + "/" + file
    if not os.path.exists(destination_file):
...
def process_other(file_location, tag):
    ...
    destination_path = LIBRARY_PATH + "/Day " + DAY + "/" + "Other"
    if not os.path.exists(destination_path):
        os.makedirs(destination_path)
    destination_file = destination_path + "/" + file
    if not os.path.exists(destination_file):
...
if __name__ == '__main__':
    ...
    if is_Copy:
        ...
        for file in scan_folder(LR_RAW_PATH):
            this_file_name = file.split(".")[0]
            this_file_type = file.split(".")[1]
            file_location = LR_RAW_PATH + "/" + file

            if this_file_type in RAW_support_types:
                process_Lr_raw(file_location)
